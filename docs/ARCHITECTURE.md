...
│   │ dashboard.php │   │ PluginM365       │   │ GLPI / e-mail /  │  │
...
- **Domínio (`inc/`)** — classes `CommonDBTM`/`CommonGLPI`. `PluginM365GraphClient` isola toda a comunicação HTTP; as demais são regras de negócio e persistência.
- **Persistência** — 7 tabelas MySQL (ver [DATABASE.md](DATABASE.md)).
- **Segurança** — `PluginM365Config` criptografa Client Secret e cache de token com AES-256-CBC; chave derivada da `GLPIKEY`.
- **Cron (`PluginM365CronTask`)** — orquestra sincronização, auditoria e consolidação de custos.
...
Cron GLPI dispara cronSyncLicenses
   ...
   └─> PluginM365License.syncFromGraph()  → upsert por sku_id
   └─> SyncLog(success)

Cron GLPI dispara cronSyncUsers
   └─> GraphClient.fetchUsers()  (streaming por página, $top=999)
        └─> por usuário:
             ├─ PluginM365User.syncFromGraph()  → upsert por azure_id
             │    └─ match glpi_users (email/UPN)
             └─ PluginM365UserLicense.syncForUser()  → reconstrói N:N
...
Cron GLPI dispara cronMonthlyReport
   └─> upsert glpi_plugin_m365_costs por (licença, período YYYY-MM)
```
...
- Falhas registradas em `glpi_plugin_m365_synclogs` com `status=error` e mensagem.
