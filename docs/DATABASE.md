...
```
                       ┌──────────────────────────┐
                       │   glpi_plugin_m365license_configs│  (linha única id=1)
                       │  tenant_id, client_id,    │
                       │  client_secret*, token*,  │
                       │  thresholds, webhook...   │
                       └──────────────────────────┘
...
┌────────────────────────┐        ┌──────────────────────────────┐
│ glpi_plugin_m365license_users │        │  glpi_plugin_m365license_licenses    │
│ id (PK)                │        │  id (PK)                      │
...
└───────────┬────────────┘                    │
            │                                  │
            │   ┌──────────────────────────────────────────┐
            └──►│  glpi_plugin_m365license_userlicenses (N:N)      │◄──┘
                │  plugin_m365license_users_id (FK)                │
                │  plugin_m365license_licenses_id (FK)             │
                │  UNIQUE(users_id, licenses_id)            │
                └──────────────────────────────────────────┘

┌───────────────────────────┐      ┌──────────────────────────┐
│ glpi_plugin_m365license_costs    │      │ glpi_plugin_m365license_alerts   │
...
└───────────────────────────┘      │ is_notified, is_resolved  │
                                   └──────────────────────────┘
┌───────────────────────────┐
│ glpi_plugin_m365license_synclogs │  sync_type, status, counters, duration_ms
└───────────────────────────┘
...
> O SQL completo de criação está em [`hook.php`](../hook.php) (`plugin_m365license_install()`), seguindo o padrão de `Migration` do GLPI (charset/collation/sign dinâmicos).
