...
class PluginM365licenseDashboard extends CommonGLPI {

    public static function getTypeName($nb = 0) {
        return __('Dashboard M365', 'm365license');
    }
    ...
    public static function collect(): array {
        global $DB;
        $lic = PluginM365licenseLicense::getGlobalStats();

        $totalUsers = 0;
        foreach ($DB->request(['SELECT' => ['COUNT' => '* AS c'], 'FROM' => 'glpi_plugin_m365license_users',
                               'WHERE' => ['is_deleted' => 0]]) as $r) {
            $totalUsers = (int)$r['c'];
        }

        $openAlerts = 0;
        foreach ($DB->request(['SELECT' => ['COUNT' => '* AS c'], 'FROM' => 'glpi_plugin_m365license_alerts',
        ...
        return [
            ...
            'by_department' => PluginM365licenseUser::countByDepartment(),
    ...
    public static function licensesByType(): array {
        global $DB;
        $out = [];
        foreach ($DB->request(['FROM' => 'glpi_plugin_m365license_licenses',
    ...
    public static function consumptionTrend(): array {
        ...
        $it = $DB->request([
            ...
            'FROM'    => 'glpi_plugin_m365license_costs',
            'GROUPBY' => 'period',
            'ORDER'   => 'period ASC',
        ]);
        foreach ($it as $row) {
    ...
    public static function show(): void {
        ...
        self::card(__('Usuários', 'm365license'), $c['total_users'], 'ti-users', 'primary');
        self::card(__('Licenças totais', 'm365license'), $c['total_licenses'], 'ti-license', 'info');
        self::card(__('Em uso', 'm365license'), $c['used_licenses'], 'ti-circle-check', 'success');
        self::card(__('Disponíveis', 'm365license'), $c['free_licenses'], 'ti-circle-dashed', 'secondary');
        self::card(__('Ociosas', 'm365license'), $c['idle_licenses'], 'ti-alert-triangle', 'warning');
        self::card(__('Alertas abertos', 'm365license'), $c['open_alerts'], 'ti-bell', 'danger');
        echo "</div>";

        // KPIs financeiros
        echo "<div class='row row-cards mt-3'>";
        self::card(__('Custo mensal', 'm365license'), $fmt($f['monthly_cost']), 'ti-currency-dollar', 'primary');
        self::card(__('Custo anual', 'm365license'), $fmt($f['annual_cost']), 'ti-calendar', 'info');
        self::card(__('Desperdício/mês', 'm365license'), $fmt($f['wasted_month']), 'ti-trash', 'danger');
        self::card(__('Economia potencial/ano', 'm365license'), $fmt($f['wasted_year']), 'ti-pig-money', 'success');
        echo "</div>";
        ...
        echo "<div class='col-md-6'><div class='card'><div class='card-header'>"
           . __('Licenças por tipo', 'm365license') . "</div><div class='card-body'>"
           . "<canvas id='m365ChartType' data-values='$byType'></canvas></div></div></div>";
        echo "<div class='col-md-6'><div class='card'><div class='card-header'>"
           . __('Usuários por departamento', 'm365license') . "</div><div class='card-body'>"
        ...
        echo "<div class='row mt-4'><div class='col-12'><div class='card'><div class='card-header'>"
           . __('Evolução do consumo (R$)', 'm365license') . "</div><div class='card-body'>"
