...
if (isset($_POST['update'])) {
    ...
    Session::addMessageAfterRedirect(__('Configuração salva.', 'm365'), true, INFO);
    Html::back();
}
...
if (isset($_POST['test'])) {
    ...
    $config->updateConfig($_POST);
    $client = new PluginM365GraphClient(PluginM365Config::getInstance());
...
echo "<div class='card-header'><h3>" . __('Integração Microsoft Graph', 'm365') . "</h3></div>";
...
echo "<tr class='tab_bg_1'><td>" . __('Abrir ticket automático (alertas críticos)', 'm365') . "</td><td>";
Dropdown::showYesNo('auto_create_ticket', $f['auto_create_ticket']);
echo "</td></tr>";

if (!empty($f['last_sync'])) {
    echo "<tr class='tab_bg_2'><td>" . __('Última sincronização', 'm365') . "</td>";
...
echo "<button type='submit' name='test' class='btn btn-outline-primary me-2'>"
   . "<i class='ti ti-plug'></i> " . __('Testar conexão', 'm365') . "</button>";
echo "<button type='submit' name='update' class='btn btn-primary'>"
   . "<i class='ti ti-device-floppy'></i> " . __('Salvar', 'm365') . "</button>";
echo "</div>";
